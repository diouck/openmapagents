...
async def _execute_plan(plan: dict, context: dict) -> dict:
    ...
    try:
        ...
        if plan.get("_buffer_m"):
            ...
            # Propager aussi dans metadata pour que App.jsx puisse le lire via action.data
            if isinstance(primary_result.get("metadata"), dict):
                primary_result["metadata"]["_buffer_m"] = plan["_buffer_m"]
            if isinstance((primary_result.get("geojson") or {}).get("metadata"), dict):
                primary_result["geojson"]["metadata"]["_buffer_m"] = plan["_buffer_m"]
...
class Orchestrator:
    ...
    async def run(
        ...
        log.info(f"[1quinquies] _is_distance_reply={_is_distance_reply} query={repr(_clean_q_buf[:30])}")
        if _is_distance_reply:
            ...
            if _route_layer_found:
                ...
                # Construire le plan directement — bypass _resolve_clip_layer
                # qui redemanderait la distance (boucle infinie)
                _clip_name  = _route_layer_found.get("name", "Itinéraire")
                _clip_bbox  = _route_layer_found.get("bbox")
                _cat_label  = "Commerces"  # défaut — affinement via query originale
                _orig_lower = (_orig_q or "").lower()
                _CATS_BUF = [
                    ("restaurant", ["restaurant","resto","brasserie"]),
                    ("school",     ["école","ecole","collège","lycée"]),
                    ("pharmacy",   ["pharmacie"]),
                ...
                _cat_code = None
                for _cc, _kws in _CATS_BUF:
                    if any(kw in _orig_lower for kw in _kws):
                        _cat_code  = _cc
                        _cat_label = _cc.replace("_"," ").title()
                        break
                if not _cat_code and any(w in _orig_lower for w in ["commerce","magasin","boutique","shop"]):
                    _cat_label = "Commerces"

                _layer_nm = f"{_cat_label} — {_clip_name} ({_buf_m}m)"
                _pad_deg  = _buf_m / 111000
                _plan_args = {
                    "theme":         "places",
                    "clip_to_layer": _clip_name,
                    "limit":         2000,
                    "_buffer_m":     _buf_m,
                    "_clip_is_route": True,
                    "_layer_name":   _layer_nm,
                }
                if _cat_code:
                    _plan_args["category"] = _cat_code
                if _clip_bbox and len(_clip_bbox) == 4:
                    dx = (_clip_bbox[2] - _clip_bbox[0]) * 0.1 + _pad_deg
                    dy = (_clip_bbox[3] - _clip_bbox[1]) * 0.1 + _pad_deg
                    _plan_args.update({
                        "bbox": _clip_bbox,
                        "xmin": _clip_bbox[0] - dx,
                        "ymin": _clip_bbox[1] - dy,
                        "xmax": _clip_bbox[2] + dx,
                        "ymax": _clip_bbox[3] + dy,
                    })
                ...
                _plan_buf = {
                    ...
                    "secondary_steps": [],
                    "_geocoded_place": _clip_name,
                    "_layer_name":   _layer_nm,
                    "_buffer_m":     _buf_m,
                }
                exec_out = await _execute_plan(_plan_buf, context)
                ...
                nl = "\n"
                text = (
                    f"**{_cat_label}** dans **{_buf_m}m** autour de **{_clip_name}**." + nl +
                    f"_Couche : {_layer_nm}_"
                ) if n else (
                    f"⚠️ Aucun **{_cat_label.lower()}** trouvé dans {_buf_m}m autour de **{_clip_name}**.\n"
                    f"_Essayez un rayon plus grand (ex: 500m) ou une autre catégorie._"
                )
                for tr in exec_out.get("tool_results", []):
                    if isinstance(tr, dict):
                        tr["layer_name"] = _layer_nm
                        tr["_buffer_m"]  = _buf_m
                try:
        ...
        if _pending_routing:
            ...
            if _resolved_center:
                ...
                _layer_name = f"Isochrone_{_intervals[0]}–{_intervals[-1]}min_{_prof_label}_{query.strip().replace(' ', '_')}"
    ...
    def _resolve_clip_layer(
        ...
        layer_name = f"{cat_label} — {clip_name}"
