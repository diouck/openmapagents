...
class RoutingAgent:
    ...
    async def _isochrone(self, client, minutes, profile, center, place) -> dict:
        ...
        prof_label_slug = prof_label.replace(" ", "_")
        loc_slug        = f"_{_slugify(place)}" if place else ""
        layer_name = (
            f"Isochrone_{intervals[0]}–{intervals[-1]}min_{prof_label_slug}{loc_slug}"
            if len(intervals) > 1 else
            f"Isochrone_{minutes}min_{prof_label_slug}{loc_slug}"
        )
    ...
    async def _isochrones_multi(self, client, intervals, profile, center, place) -> dict:
        ...
        prof_label_slug = prof_label.replace(" ", "_")
        loc_slug        = f"_{_slugify(place)}" if place else ""
        layer_name      = f"Isochrones_{intervals[0]}–{intervals[-1]}min_{prof_label_slug}{loc_slug}"
    ...
    async def _route(self, client, query, profile, from_place, to_place, context) -> dict:
        ...
        _prof_label_slug = {"foot":"à_pied","bike":"à_vélo","car":"en_voiture"}.get(profile, profile)
        _layer_name      = f"Itinéraire_{_from_slug}_vers_{_to_slug}_{_prof_label_slug}"
