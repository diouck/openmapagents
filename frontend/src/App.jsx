      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <div style={{flex:1,position:"relative",minWidth:0,background:globeOn?"#010206":undefined}}>
          ...
          <Map ref={mapRef} {...vs} onMove={e=>setVs(e.viewState)}
            ...
            {layers.map(l=>l.visible&&!l.isRaster&&l.extrude&&(<Source key={`${l.id}-3d`} id={`${l.id}-3d`} type="geojson" data={l.geojson}><Layer id={`${l.id}-extrude`} type="fill-extrusion" filter={["any",["==",["geometry-type"],"Polygon"],["==",["geometry-type"],"MultiPolygon"]]} slot={MB_SLOT} paint={{"fill-extrusion-color":l.classResult?.expression||l.color,"fill-extrusion-height":l.extrudeAttr?["*",["to-number",["get",l.extrudeAttr],9],l.extrudeScale||1]:["*",["to-number",["get","height"],["get","render_height"],9],l.extrudeScale||1],"fill-extrusion-base":["to-number",["get","min_height"],["get","render_min_height"],0],"fill-extrusion-opacity":l.opacity*0.85}}/>{l.labels&&<Layer id={`${l.id}-3dlabel`} type="symbol" layout={{"text-field":["get",l.labelAttr||"name"],"text-size":10,"text-anchor":"center"}} paint={{"text-color":"#fff","text-halo-color":"#000","text-halo-width":1}}/>}</Source>))}
