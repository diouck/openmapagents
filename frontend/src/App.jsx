...
// Slot Mapbox : place les couches de données AU-DESSUS du fond (styles v3/Standard à
// « slots », qui placent sinon les couches custom sous le fond). undefined en MapLibre.
...
import { loadShapeMarker } from "./utils/shapeMarker";
...
export default function App() {
  ...
  // Fermeture de l'outil « Ombres » → carte vierge : on repasse en mode pointeur, on
  // déverrouille les fonds et on RETIRE toutes les couches/sources de l'outil (préfixe
  // « oma- ») + le relief. Certains ajouts sont asynchrones (canopée GEE, compute rAF) et
  // peuvent survenir juste APRÈS la fermeture → on repasse plusieurs fois (idempotent).
  ...
  useEffect(() => {
    ...
    if (shadowWasOpenRef.current && !open) {
      ...
      const purge = () => {
        if (openPanelsRef.current?.has?.("shadow")) return;      // rouvert entre-temps → ne pas purger
        const map = mapRef.current?.getMap?.(); if (!map) return;
        try { map.setTerrain(null); } catch (_) {}
        try {
          const st = map.getStyle?.(); if (!st) return;
          (st.layers || []).forEach(l => { if (l.id && l.id.indexOf("oma-") === 0 && map.getLayer(l.id)) { try { map.removeLayer(l.id); } catch (_) {} } });
          Object.keys(st.sources || {}).forEach(sid => { if (sid.indexOf("oma-") === 0 && map.getSource(sid)) { try { map.removeSource(sid); } catch (_) {} } });
        } catch (_) {}
      };
      purge();
      [250, 700, 1500, 3000, 5000].forEach(ms => setTimeout(purge, ms));   // rattrape les ré-ajouts async tardifs (canopée GEE lente)
      try { mapRef.current?.getMap?.()?.easeTo({ pitch: 0, bearing: 0, duration: 300 }); } catch (_) {}
    }
  ...
  useEffect(() => {
    ...
    const tryLoad = () => {
      ...
      layersRef.current.forEach(l => {
        ...
        if (l.markerShape && l.markerShape !== "circle") {
          const id = loadShapeMarker(map, l.markerShape, l.color, l.outlineColor || l.color, l.strokeWidth ?? 1.5, (l.radius || 5) * 2);
          if (id && map.hasImage(id)) loaded++;
        }
      });
  ...
  useEffect(() => {
    ...
    layers.forEach(l => {
      ...
      if (l.markerShape && l.markerShape !== "circle") {
        loadShapeMarker(map, l.markerShape, l.color, l.outlineColor || l.color, l.strokeWidth ?? 1.5, (l.radius || 5) * 2);
      }
    });
  }, [layers]);
  ...
  useEffect(() => {
    ...
    layers.forEach(l => {
      if (l.isRaster) {
        ...
        orderedIds.push(`${l.id}-fill`, `${l.id}-outline`, `${l.id}-road`, `${l.id}-circle`, `${l.id}-mk`);
  ...
  const getPaint = useCallback((layer, gt) => {
    ...
    if (gt==="line") { if (cr?.type==="proportional_line"&&cr.widthExpression) return {"line-color":layer.color,"line-width":cr.widthExpression,"line-opacity":layer.opacity}; return {"line-color":ce,"line-width":layer.sizeResult?.widthExpression??(layer.strokeWidth??1.5),"line-opacity":layer.opacity}; }
    if (gt==="circle") { if (layer.markerShape&&layer.markerShape!=="circle") return {"circle-radius":0,"circle-opacity":0}; if (cr?.type==="symbol"&&cr.symbolMode==="maki") { const _map=mapRef.current?.getMap?.(); const _mkSz=parseInt(cr.makiSize)||30; const _imgId=cr.makiImageId||(`maki_${cr.makiName}_${(cr.makiColor||"#ffffff").replace("#","").toLowerCase()}_${_mkSz}`); const _imgReady=_imgId&&_map&&_map.isStyleLoaded()&&_map.hasImage(_imgId); if(_imgReady) return {"circle-radius":0,"circle-opacity":0}; return {"circle-radius":Math.max(4,_mkSz/6),"circle-color":cr.makiColor||layer.color,"circle-opacity":layer.opacity,"circle-stroke-width":1,"circle-stroke-color":"#fff","circle-stroke-opacity":0.3}; } if(cr?.type==="symbol"&&cr.symbolMode==="image") return {"circle-radius":0,"circle-opacity":0}; if (cr?.type==="proportional"&&cr.radiusExpression) return {"circle-radius":cr.radiusExpression,"circle-color":cr.expression||layer.color,"circle-opacity":layer.opacity,"circle-stroke-width":1,"circle-stroke-color":"#fff","circle-stroke-opacity":0.4}; return {"circle-radius":layer.sizeResult?.radiusExpression??(layer.radius||5),"circle-color":ce,"circle-opacity":layer.opacity,"circle-stroke-width":1,"circle-stroke-color":"#fff","circle-stroke-opacity":0.3}; }
    return {};
  }, []);
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <div style={{flex:1,position:"relative",minWidth:0,background:globeOn?"#010206":undefined}}>
          ...
          <Map ref={mapRef} {...vs} onMove={e=>setVs(e.viewState)}
            ...
            {layers.map(l=>{if(!l.isRaster)return null;if(l.kind==="image")return(<Source key={`${l.id}-${l.styleV||0}`} id={l.id} type="image" url={l.imageUrl} coordinates={l.coordinates}><Layer id={`${l.id}-layer`} type="raster" slot={MB_SLOT} layout={{visibility:l.visible?"visible":"none"}} paint={{"raster-opacity":l.opacity??0.85,"raster-fade-duration":0}}/></Source>);if(l.theme==="vector")return(<Source key={l.id} id={l.id} type="vector" tiles={[l.tileUrl]} minzoom={0} maxzoom={22}><Layer id={`${l.id}-fill`} type="fill" slot={MB_SLOT} layout={{visibility:l.visible?"visible":"none"}} filter={["==",["geometry-type"],"Polygon"]} paint={{"fill-color":l.color||C.acc,"fill-opacity":l.opacity??0.3}}/><Layer id={`${l.id}-line`} type="line" slot={MB_SLOT} layout={{visibility:l.visible?"visible":"none"}} filter={["any",["==",["geometry-type"],"LineString"],["==",["geometry-type"],"Polygon"]]} paint={{"line-color":l.color||C.acc,"line-width":1.5,"line-opacity":l.opacity??1}}/><Layer id={`${l.id}-circle`} type="circle" slot={MB_SLOT} layout={{visibility:l.visible?"visible":"none"}} filter={["==",["geometry-type"],"Point"]} paint={{"circle-color":l.color||C.acc,"circle-radius":4,"circle-stroke-width":1,"circle-stroke-color":"#fff","circle-opacity":l.opacity??1}}/></Source>);return(<Source key={l.id} id={l.id} type="raster" tiles={[l.tileUrl]} tileSize={256}><Layer id={`${l.id}-layer`} type="raster" slot={MB_SLOT} layout={{visibility:(l.visible&&l.id!==tlLayerId)?"visible":"none"}} paint={{"raster-opacity":l.opacity??0.85}}/></Source>);})}
            {layers.map(l=>l.visible&&!l.isRaster&&l.geojson&&!l.heatmap&&!l.extrude&&!l.cluster&&(<Source key={l.id} id={l.id} type="geojson" data={l.geojson}><Layer id={`${l.id}-fill`} type="fill" filter={["any",["==",["geometry-type"],"Polygon"],["==",["geometry-type"],"MultiPolygon"]]} minzoom={l.minZoom??undefined} maxzoom={l.maxZoom??undefined} slot={MB_SLOT} paint={getPaint(l,"fill")}/><Layer id={`${l.id}-outline`} type="line" minzoom={l.minZoom??undefined} maxzoom={l.maxZoom??undefined} filter={["any",["==",["geometry-type"],"Polygon"],["==",["geometry-type"],"MultiPolygon"]]} layout={{"line-join":l.lineJoin||"miter","line-cap":l.lineCap||"butt","line-miter-limit":8}} slot={MB_SLOT} paint={getPaint(l,"outline")}/><Layer id={`${l.id}-road`} type="line" minzoom={l.minZoom??undefined} maxzoom={l.maxZoom??undefined} filter={["==",["geometry-type"],"LineString"]} layout={{"line-join":l.lineJoin||"miter","line-cap":l.lineCap||"butt","line-miter-limit":8}} slot={MB_SLOT} paint={getPaint(l,"line")}/><Layer id={`${l.id}-circle`} type="circle" minzoom={l.minZoom??undefined} maxzoom={l.maxZoom??undefined} filter={["==",["geometry-type"],"Point"]} slot={MB_SLOT} paint={getPaint(l,"circle")}/>{l.markerShape&&l.markerShape!=="circle"&&(()=>{void makiTick;const map=mapRef.current?.getMap?.();const imgId=map?loadShapeMarker(map,l.markerShape,l.color,l.outlineColor||l.color,l.strokeWidth??1.5,(l.radius||5)*2):null;if(!imgId)return null;const _sr=l.sizeResult,_bR=l.radius||5;const _isz=(_sr?.radiusExpression&&_sr.mode==="radius")?["interpolate",["linear"],["to-number",["get",_sr.attribute],0],_sr.minVal,_sr.minSize/_bR,_sr.maxVal===_sr.minVal?_sr.maxVal+1:_sr.maxVal,_sr.maxSize/_bR]:1;return <Layer id={`${l.id}-mk`} type="symbol" filter={["==",["geometry-type"],"Point"]} slot={MB_SLOT} layout={{"icon-image":imgId,"icon-size":_isz,"icon-allow-overlap":true,"icon-ignore-placement":true,"icon-anchor":"center"}} paint={{"icon-opacity":l.opacity??1}}/>;})()}{l.labels&&<Layer id={`${l.id}-label`} type="symbol" slot={MB_SLOT} layout={{"text-field":["get",l.labelAttr||"name"],"text-size":l.labelSize||11,"text-offset":[0,1.2],"text-anchor":"top","text-max-width":10,"text-allow-overlap":!(l.labelAvoidOverlap??true)}} paint={{"text-color":l.color,"text-halo-color":"#fff","text-halo-width":l.labelHalo??1.5}}/>}{l.classResult?.type==="symbol"&&(()=>{const cr=l.classResult;const map=mapRef.current?.getMap?.();if(cr.symbolMode==="image"&&cr.customImage?.id)return<Layer id={`${l.id}-icon`} type="symbol" filter={["==",["geometry-type"],"Point"]} layout={{"icon-image":cr.customImage.id,"icon-size":cr.imageSize||1,"icon-allow-overlap":true,"icon-anchor":"center"}} paint={{"icon-opacity":l.opacity}}/>;if(cr.symbolMode==="maki"&&cr.makiName&&map){const _mkSize=parseInt(cr.makiSize)||30;void makiTick;const imgId=loadMakiIcon(map,cr.makiName,cr.makiColor||"#ffffff",_mkSize);if(!imgId)return<Layer id={`${l.id}-sym-fb`} type="circle" filter={["==",["geometry-type"],"Point"]} paint={{"circle-radius":5,"circle-color":cr.makiColor||l.color,"circle-opacity":l.opacity,"circle-stroke-width":1,"circle-stroke-color":"#fff"}}/>;return<Layer id={`${l.id}-sym`} type="symbol" filter={["==",["geometry-type"],"Point"]} layout={{"icon-image":imgId,"icon-size":1,"icon-allow-overlap":true,"icon-ignore-placement":true,"icon-anchor":"center"}} paint={{"icon-opacity":l.opacity}}/>;}return null;})()}</Source>))}
            {/* Ronds proportionnels au centroïde des polygones (2e style "taille") */}
            {layers.map(l=>l.visible&&!l.isRaster&&l.sizeResult?.bubbles&&!l.heatmap&&!l.extrude&&!l.cluster&&(<Source key={`${l.id}-bub`} id={`${l.id}-bub`} type="geojson" data={l.sizeResult.bubbles}><Layer id={`${l.id}-bubble`} type="circle" minzoom={l.minZoom??undefined} maxzoom={l.maxZoom??undefined} slot={MB_SLOT} paint={{"circle-radius":l.sizeResult.radiusExpression,"circle-color":l.sizeResult.color||l.classResult?.expression||l.color,"circle-opacity":l.opacity??1,"circle-stroke-width":1,"circle-stroke-color":"#fff","circle-stroke-opacity":0.6}}/></Source>))}
            {layers.map(l=>l.visible&&!l.isRaster&&l.extrude&&(<Source key={`${l.id}-3d`} id={`${l.id}-3d`} type="geojson" data={l.geojson}><Layer id={`${l.id}-extrude`} type="fill-extrusion" filter={["any",["==",["geometry-type"],"Polygon"],["==",["geometry-type"],"MultiPolygon"]]} slot={MB_SLOT} paint={{"fill-extrusion-color":l.classResult?.expression||l.color,"fill-extrusion-height":l.extrudeAttr?["*",["to-number",["get",l.extrudeAttr],5],l.extrudeScale||1]:["*",["to-number",["get","height"],["get","render_height"],["*",["to-number",["get","building:levels"],["get","levels"],2],3],8],l.extrudeScale||1],"fill-extrusion-base":["to-number",["get","min_height"],["get","render_min_height"],0],"fill-extrusion-opacity":l.opacity*0.85}}/>{l.labels&&<Layer id={`${l.id}-3dlabel`} type="symbol" layout={{"text-field":["get",l.labelAttr||"name"],"text-size":10,"text-anchor":"center"}} paint={{"text-color":"#fff","text-halo-color":"#000","text-halo-width":1}}/>}</Source>))}
            {layers.map(l=>l.visible&&!l.isRaster&&l.cluster&&(<Source key={`${l.id}-cl`} id={`${l.id}-cl`} type="geojson" data={l.geojson} cluster={true} clusterMaxZoom={14} clusterRadius={l.clusterRadius||50}><Layer id={`${l.id}-clusters`} type="circle" filter={["has","point_count"]} slot={MB_SLOT} paint={{"circle-color":["step",["get","point_count"],l.color,10,C.amb,50,C.red],"circle-radius":["step",["get","point_count"],18,10,24,50,32],"circle-opacity":0.85,"circle-stroke-width":2,"circle-stroke-color":"#fff"}}/><Layer id={`${l.id}-cluster-count`} type="symbol" filter={["has","point_count"]} slot={MB_SLOT} layout={{"text-field":"{point_count_abbreviated}","text-size":12}} paint={{"text-color":"#fff"}}/><Layer id={`${l.id}-unclustered`} type="circle" filter={["!",["has","point_count"]]} slot={MB_SLOT} paint={{"circle-radius":l.radius||5,"circle-color":l.color,"circle-opacity":l.opacity,"circle-stroke-width":1,"circle-stroke-color":"#fff","circle-stroke-opacity":0.3}}/></Source>))}
